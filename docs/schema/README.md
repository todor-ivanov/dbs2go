...
[Architecture](#architecture) · [Relational maps](#relational-maps) · [Foreign keys](#foreign-key-registry) · [Table dictionary](#table-dictionary) · [Other database objects](#other-database-objects)

Legend: `PK` primary key · `FK` foreign key · `UQ` unique key · `NN` not null · arrows run from referencing column to referenced column.

## Architecture

The schema has five functional areas. The central persisted-data path is dataset → block → file → luminosity section; lookup tables classify those records, parentage tables record provenance, configuration tables describe producing software, and migration tables track transfers.
...
This overview is intentionally table-level. Open the domain maps below for exact column endpoints.
...
flowchart LR
  ...
  n_table_ASSOCIATED_FILES -->|"FL_AF"| n_table_FILES
  n_table_ASSOCIATED_FILES -->|"FL_AF2"| n_table_FILES
  n_table_BLOCKS -->|"DS_BK"| n_table_DATASETS
  n_table_BLOCK_PARENTS -->|"BK_BP"| n_table_BLOCKS
  n_table_BLOCK_PARENTS -->|"BK_BP2"| n_table_BLOCKS
  ...
  n_table_DATASET_PARENTS -->|"DS_DP"| n_table_DATASETS
  n_table_DATASET_PARENTS -->|"DS_DP2"| n_table_DATASETS
  ...
  n_table_FILE_PARENTS -->|"FL_FP"| n_table_FILES
  n_table_FILE_PARENTS -->|"FL_FP2"| n_table_FILES
...
Each domain map renders tables with their columns and connects every originating foreign key to its exact referenced column. The maps are folded to keep the GitHub page compact.

<details>
<summary><strong>Core data and containment</strong> — 5 primary tables, 5 foreign keys</summary>
...
flowchart LR
  ...
  subgraph n_sg_DATASETS["DATASETS · PK PK_DS"]
    ...
    n_DATASETS__PRIMARY_DS_ID["PRIMARY_DS_ID<br/>INTEGER · NN"]
    n_DATASETS__PROCESSED_DS_ID["PROCESSED_DS_ID<br/>INTEGER · NN"]
    n_DATASETS__DATA_TIER_ID["DATA_TIER_ID<br/>INTEGER · NN"]
    n_DATASETS__DATASET_ACCESS_TYPE_ID["DATASET_ACCESS_TYPE_ID<br/>INTEGER · NN"]
    n_DATASETS__ACQUISITION_ERA_ID["ACQUISITION_ERA_ID<br/>INTEGER"]
    n_DATASETS__PROCESSING_ERA_ID["PROCESSING_ERA_ID<br/>INTEGER"]
    n_DATASETS__PHYSICS_GROUP_ID["PHYSICS_GROUP_ID<br/>INTEGER"]
  ...
  subgraph n_sg_FILES["FILES · PK PK_FL"]
    ...
    n_FILES__FILE_TYPE_ID["FILE_TYPE_ID<br/>INTEGER · NN"]
    ...
    n_FILES__BRANCH_HASH_ID["BRANCH_HASH_ID<br/>INTEGER"]
...
<details>
<summary><strong>Classification and lookup</strong> — 10 primary tables, 10 foreign keys</summary>
...
<details>
<summary><strong>Parentage and associations</strong> — 4 primary tables, 8 foreign keys</summary>
...
<details>
<summary><strong>Processing configuration</strong> — 6 primary tables, 7 foreign keys</summary>
...
<details>
<summary><strong>Migration and instance metadata</strong> — 3 primary tables, 1 foreign key</summary>
